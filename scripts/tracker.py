...
# Lines generated by delivery/telemetry layers rather than the legal document itself.
# These are ignored only for deciding whether a diff is publishable; the validated
# snapshot is still refreshed so the same technical variation is not seen again.
TECHNICAL_LINE_PATTERNS = [
    r"^this is the trace id:\\s*[a-f0-9-]+$",
    r"^trace id:\\s*[a-f0-9-]+$",
...
def is_technical_line(line):
    ...
    return any(re.search(pattern, value, re.IGNORECASE) for pattern in TECHNICAL_LINE_PATTERNS)

def substantive_changed_lines(added_lines, removed_lines):
    return (
        [line for line in added_lines if not is_technical_line(line)],
        [line for line in removed_lines if not is_technical_line(line)],
    )
...
def main():
    ...
    for target in targets:
        ...
        substantive_added, substantive_removed = substantive_changed_lines(added_lines, removed_lines)
        if not substantive_added and not substantive_removed:
            source_status["state"] = "technical_change_ignored"
            source_status["ignored_technical_change"] = True
            with open(filepath, "w", encoding="utf-8") as f:
                f.write(new_text)
            status["sources"].append(source_status)
            continue
        categories = classify_categories(substantive_added, substantive_removed)
        relevance = classify_relevance(categories, len(substantive_added), len(substantive_removed))
        raw_diff = "".join(diff[:MAX_DIFF_LINES])
        cid = change_id(policy_id, current_date, raw_diff)
        diff_log.insert(0, {"schema_version": SCHEMA_VERSION, "classifier_version": CLASSIFIER_VERSION, "id": policy_id, "change_id": cid, "permalink": f"{SITE_URL}#change-{cid}", "platform": target["platform"], "type": document_type, "document_type": document_type, "url": target["url"], "date": current_date, "severity": relevance, "relevance": relevance, "categories": categories, "impacts": categories, "added_count": len(substantive_added), "removed_count": len(substantive_removed), "summary": build_plain_summary(categories, len(substantive_added), len(substantive_removed)), "raw_diff": raw_diff})
        status["changes"] += 1
        source_status.update({"changed": True, "change_id": cid, "relevance": relevance, "categories": categories, "added_count": len(substantive_added), "removed_count": len(substantive_removed)})
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(new_text)
        status["sources"].append(source_status)
